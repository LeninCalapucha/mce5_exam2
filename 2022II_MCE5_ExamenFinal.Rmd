...
Cuando los datos son de importancia 
...
Datos_oso_anteojos= filter(occ$data, country == 'Ecuador')


shp = raster::getData('GADM', country = 'ECU', level = 1)

 ggplot() +
  geom_point(data = Datos_oso_anteojos, aes(x = decimalLongitude, y = decimalLatitude), color = 'black') +
  geom_polygon(data = shp, aes(x = long, y = lat, group = group), color = 'grey', fill = NA) +
  coord_fixed(ylim = c(-5, 2.5), xlim = c(-92.5, -30))+  xlab('Longitud') +
 ylab('Latitud')
```
